...
describe BulkUpload do
  ...
  let(:repository_name){ 'products repository' }
  ...
  describe '#enqueue_batch' do
    ...
    it 'enqueues processing into BulkUploadJob' do
      bulk_upload.enqueue_batch(
        processable_content: processable_content, upload_type: upload_type, account_id: 42, repository_name: repository_name
      )
      expect(BulkUploadJob).to have_received(:perform_later)
        .with(batch_upload.id, processable_content, repository_name)
    end
  ...
  describe '#handle_batch' do
    ...
    it 'asks handlers factory to creates json handler' do
      bulk_upload.handle_batch(batch_upload.id, processable_content, repository_name)
